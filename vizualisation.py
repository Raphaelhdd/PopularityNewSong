...
def plot_audio_features_vs_popularity(tracks_df):
    ...
    for i, feature in enumerate(audio_features):
        plt.subplot(3, 3, i + 1)
        sns.scatterplot(x=feature, y='popularity', data=tracks_df)
        plt.title(f'{feature.capitalize()} vs Popularity')
        plt.xlabel(feature.capitalize())
        plt.ylabel('Popularity Index')
    plt.tight_layout()
    plt.savefig('audio_features_vs_popularity.png')
    plt.show()


def plot_popularity_by_album_type(tracks_df, artist_df):
    """
